...
URL Maps: ${lead.maps_url}`;
    ...
    // Use OpenRouter API for better reliability and IP rotation
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
        'HTTP-Referer': 'https://leadmachinepro.com',
    ...
    if (response.status === 402) {
      return { success: false, error: 'Créditos insuficientes no OpenRouter' };
    }
    ...
    if (!response.ok) {
      ...
      if (attempt < RETRY_ATTEMPTS) {
        const delay = RETRY_DELAY_MS * Math.pow(2, attempt - 1);
        console.log(`API error on ${lead.business_name}, retrying in ${delay}ms (attempt ${attempt}): ${errorText}`);
        await sleep(delay);
        return analyzeSingleLead(lead, apiKey, attempt + 1);
      }
    ...
    if (!analysisText) {
      console.error(`Resposta vazia para ${lead.business_name}!`, data);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Using OpenRouter API for better reliability`);
    ...
    const analysisPromises = leads.map((lead: Lead, index: number) => async () => {
      ...
      const analysis = await analyzeSingleLead(lead, OPENROUTER_API_KEY);
